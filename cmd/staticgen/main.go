...
func main() {
	tlsEnabled = false
	log.Printf("DapperDox generator version %s starting\n", VERSION)
	...
	render.Register()
	exitOnError(func() error {
		return guides.Render(cfg.OutDir)
	})
	exitOnError(func() error {
		return specs.Render(cfg.OutDir)
	})
	exitOnError(func() error {
		return reference.Render(cfg.OutDir)
	})
	exitOnError(func() error {
		return static.Render(cfg.OutDir)
	})
	exitOnError(func() error {
		return home.Render(cfg.OutDir)
	})
...
func exitOnError(fun func() error) {
	err := fun()
	if err != nil {
